...
public class MapServer {
    ...
    public static List<String> getLocationsByPrefix(String prefix) {
        ...
        if (buffer.containsKey(prefix)) {
            return buffer.get(prefix);
        }
        LinkedList<GraphDB.Node> nodes = graph.tire.get(prefix);
        for (GraphDB.Node n : nodes) {
            if (!n.name.equals(result.peekLast())) {
                result.addLast(n.name);
            }
        }
        /*
        String reg = "^" + prefix + ".*";
        for (String key : graph.nameMap.keySet()) {
            if (key.matches(reg)) {
                for (GraphDB.Node n : graph.nameMap.get(key)) {
                    if (!n.name.equals(result.peekLast())) {
                        result.addLast(n.name);
                    }
                }
        ...
        */
        ...
        buffer.put(prefix, result);
        if (prefixQueue.size() > 100) {
            String pre = prefixQueue.remove();
            buffer.remove(pre);
        }
        return result;
    }

    private static HashMap<String, List<String>> buffer;
    private static Queue<String> prefixQueue;
    ...
    public static List<Map<String, Object>> getLocations(String locationName) {
        ...
        for (GraphDB.Node n : graph.tire.getExact(cleanName)) {
